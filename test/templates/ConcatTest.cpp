...
class ConcatTestSuite : public Test {
 protected:
  ...
  std::string expectedVal5 {"stringval"};
  std::string expectedVal6 {"string1"};
  class Empty{};
  Empty empty1{};
  Empty empty2{};
...
TEST_F(ConcatTestSuite, CallConcat) {
 ...
 EXPECT_EQ(expectedVal5, actualVal5);
 EXPECT_EQ("appleorange", concat(x, y));
}
